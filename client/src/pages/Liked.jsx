...
export default function Liked() {
  ...
  // so if an object in allLikes has a user_id === loggedAs.user.id
  // and that object's public_id === meme.publicId
  // return it
  ...
  useEffect(() => {
    allLikes &&
      setLikedMemes(
        allLikes.map((like) => {
          if (like.user_id === loggedAs.user.id) {
            return like.public_id;
          }
        })
      );
  }, [allLikes]);
      ...
      <section className="flex justify-center">
        <div className="inline-flex flex-col gap-5 my-32 ">
          {memes.map((meme, idx) => {
            return (
              likedMemes &&
              likedMemes.includes(meme.publicId) && (
